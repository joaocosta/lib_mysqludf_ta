...
You need to copy the binary library to the MySQL plugin directory.
To find out what the MySQL plugin directory is run this in MySQL prompt:
...
SELECT datetime, ( ta_sma(close,21) + 2*SQRT(ta_sum(POW(close - ta_sma(close, 21), 2), 21)/21) ) AS `BOL_UP`
FROM ( SELECT * FROM EURUSD_86400 ORDER BY datetime ASC ) AS T;

To calculate the lower limit Bollinger Band of 2 standard deviations over a 21 period sma:
SELECT datetime, ( ta_sma(close,21) - 2*SQRT(ta_sum(POW(close - ta_sma(close, 21), 2), 21)/21) ) AS `BOL_DOWN`
FROM ( SELECT * FROM EURUSD_86400 ORDER BY datetime ASC ) AS T;
...
The easiest way might be to copy one of the existing .c files to a different name and modify its implementation.
The provided Makefile will pick up new .c files.
